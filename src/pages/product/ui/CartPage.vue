<!-- src/pages/product/ui/CartPage.vue -->
    ...
    <template v-else>
      <template v-if="cart.list.length">
              ...
              >
                <td class="px-5 py-4">
                  <div class="flex items-center gap-4">
                    <img :src="i.image" alt="" class="w-16 h-16 rounded-lg object-cover ring-1 ring-gray-200" />
        ...
        <div class="mt-6 flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
          <div class="flex items-center gap-3">
            ...
            />
            <button @click="apply" class="px-4 py-2 border rounded-lg bg-white hover:bg-gray-50">Apply</button>
          ...
          <div class="text-right space-y-1">
            <p>Subtotal: <b>{{ cart.subtotal.toFixed(2) }} $</b></p>
        ...
        <div class="mt-8 flex gap-4">
          <RouterLink to="/catalog/men" class="px-4 py-2 border rounded-lg hover:bg-gray-50">Continue shopping</RouterLink>
          <button @click="proceed" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            Proceed to checkout
          </button>
        </div>
      </template>
      ...
      <template v-else>
        <div class="border border-dashed rounded-xl p-10 text-center text-gray-500 bg-gray-50">
          ...
          <RouterLink to="/catalog/men" class="text-blue-600 hover:underline">Перейти в каталог</RouterLink>
        </div>
      </template>
    </template>
  </section>
