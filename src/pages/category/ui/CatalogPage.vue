<!-- src/pages/category/ui/CatalogPage.vue -->
<script setup lang="ts">
import { ref, computed, watch, onMounted } from 'vue'
import { useRoute } from 'vue-router'
import { useProductsStore } from '@entities/product'
import { useWishlistStore, useInfiniteProducts } from '@features/product'
...
watch(
  ...
  (v) => {
    if (v) price.value = [productsStore.priceMin, productsStore.priceMax]
  },
  { immediate: true }
)
...
onMounted(async () => {
  ...
  wishlist.start()
  if (productsStore.all.length) resetAndLoad()
})

function resetFilters() {
  category.value = color.value = size.value = null
  query.value = ''
  price.value = [Math.max(0, productsStore.priceMin), Math.max(0, productsStore.priceMax)]
  resetAndLoad()
}
</script>
    ...
    >
      ...
      <div class="space-y-1">
        ...
        >
          ...
          <option v-for="c in productsStore.categories" :key="c" :value="c">{{ c }}</option>
        </select>
      </div>
      ...
      <div class="space-y-1">
        ...
        >
          <option :value="null">Все</option>
          <option v-for="c in productsStore.colors" :key="c" :value="c">{{ c }}</option>
        </select>
      </div>
      ...
      <div class="space-y-1">
        ...
        >
          <option :value="null">Все</option>
          <option v-for="s in productsStore.sizes" :key="s" :value="s">{{ s }}</option>
        </select>
      </div>
