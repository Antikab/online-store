<!-- src/pages/user/ui/ChangePasswordPage.vue -->
...
const { loading, errorMessage, email } = storeToRefs(auth)

const currentPassword = ref('')
const newPassword = ref('')
const ok = ref(false)

const canSubmit = computed(() => !loading.value && !!currentPassword.value && newPassword.value.length >= 6)
...
<template>
  <section class="max-w-md mx-auto px-4 py-10">
    ...
    <form @submit.prevent="onSubmit" novalidate class="flex flex-col gap-5 bg-white p-6 rounded-xl shadow-sm ring-1 ring-gray-200">
      <div>
