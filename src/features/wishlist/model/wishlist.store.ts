// features/wishlist/model/wishlist.store.ts
...
function mapProduct(source: Product): WishlistProduct {
  return {
    id: source.id,
    title: source.title,
    price: source.price,
...
function mapRowProduct(raw: NonNullable<WishlistRow['products']>): WishlistProduct {
  return {
    id: raw.id,
    title: raw.title,
...
export const useWishlistStore = defineStore('wishlist', () => {
  ...
  async function refresh(uid: string) {
    ...
    products.value = rows
      .map((r) => r.products)
      .filter((raw): raw is NonNullable<WishlistRow['products']> => Boolean(raw))
      .map(mapRowProduct)
