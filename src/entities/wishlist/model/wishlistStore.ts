...
type WishlistProduct = Pick<Product, 'id' | 'title' | 'price' | 'imageUrls'>

const toWishlistProduct = (product: Product): WishlistProduct => ({
  id: product.id,
  title: product.title,
  price: product.price,
  ...
  function loadGuest() {
    try {
      ...
      products.value = arr
        ...
        .filter((value): value is Product => value !== null)
  ...
  async function toggle(id: string) {
    ...
    if (isGuest.value || !uid) {
      if (ids.value.has(id)) {
        ...
        if (prod) products.value.push(toWishlistProduct(prod))
      }
      saveGuest()
      return
    }
