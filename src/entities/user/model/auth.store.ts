...
export const useAuthStore = defineStore('auth', () => {
  const { signIn, signUp, signOut, resetPassword, updatePassword, loading, errorMessage } = useAuth('Auth')
  ...
  async function changePasswordWithReauth(currentPassword: string, newPassword: string) {
    ...
    if (!currentEmail) throw new Error('auth email required')
    const data = await signIn(currentEmail, currentPassword)
    if (!data?.user) throw new Error('current password is incorrect')
