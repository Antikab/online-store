<!-- src/widgets/user/ui/AppHeader.vue -->
...
<template>
  <header class="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-white">
    ...
    <template v-if="isAuthed">
      ...
      <button @click="onLogout" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm hover:bg-gray-50">
        Выйти
      </button>
    </template>
    <template v-else>
      <RouterLink class="rounded-md border border-gray-300 px-3 py-1.5 text-sm hover:bg-gray-50" to="/login">
        Войти
      </RouterLink>
      <RouterLink class="rounded-md border border-gray-300 px-3 py-1.5 text-sm hover:bg-gray-50 ml-2" to="/register">
        Регистрация
      </RouterLink>
    </template>
  </header>
